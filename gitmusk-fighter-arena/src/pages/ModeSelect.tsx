...
export function ModeSelect() {
  ...
  // 2-minute countdown while waiting for friend — auto-expire when time runs out
  useEffect(() => {
    if (!waitingFriend) return;
    setWaitTimer(120);
    ...
    return () => { clearInterval(interval); clearTimeout(expireTimeout); };
  }, [waitingFriend]);
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="flex mb-4" style={{ border: '3px solid var(--panel-line)' }}>
          {(['random', 'friend'] as const).map(t => (
            <button key={t}
              onClick={() => { setTab(t); if (searching) leaveQueue(); setWaitingFriend(false); setInviteLink(''); setRoomExpired(false); setWaitTimer(120); }}
        ...
        {tab === 'friend' && (
          <div className="g-panel pink" style={{ padding: '24px' }}>
            ...
            {roomExpired ? (
              <div className="text-center">
                <div style={{ fontFamily: 'var(--pixel)', fontSize: '32px', color: 'var(--neon-pink)', marginBottom: '12px' }}>⏰</div>
                <div style={{ fontFamily: 'var(--pixel)', fontSize: '10px', color: 'var(--neon-pink)', marginBottom: '8px', letterSpacing: '.1em' }}>
                  LINK EXPIRED
                </div>
                <p style={{ fontFamily: 'var(--body)', fontSize: '15px', color: 'var(--txt-dim)', marginBottom: '20px', lineHeight: 1.5 }}>
                  Friend didn't join within 2 minutes. Create a new link to try again.
                </p>
                <button onClick={() => { setRoomExpired(false); setWaitTimer(120); }} className="g-btn pink full" style={{ fontSize: '11px' }}>
                  🔗 CREATE NEW LINK
                </button>
              </div>
            ) : !waitingFriend ? (
              ...
              <div className="py-2">
                ...
                <div className="text-center mb-3">
                  <div style={{
                    fontFamily: 'var(--pixel)', fontSize: '28px',
                    color: waitTimer <= 30 ? 'var(--neon-pink)' : 'var(--neon-yel)',
                    textShadow: waitTimer <= 30 ? '0 0 12px var(--neon-pink)' : '0 0 8px rgba(255,214,10,.4)',
                    letterSpacing: '.05em',
                  }}>
                    {Math.floor(waitTimer / 60)}:{String(waitTimer % 60).padStart(2, '0')}
                  </div>
                  <div style={{ fontFamily: 'var(--pixel)', fontSize: '7px', color: 'var(--txt-dim)', marginTop: '4px', letterSpacing: '.1em' }}>
                    LINK EXPIRES IN
                  </div>
                </div>
